...
def verify_artifact(artifact: Path, label: str) -> None:
    with tempfile.TemporaryDirectory(prefix=f"legends-sa3-{label}-") as temp:
        ...
        run([str(python), "-c", "import legends_sa3; assert legends_sa3.__version__ == '0.1.0'"])
        run([str(cli), "--version"])
...
def main() -> int:
    ...
    wheels = sorted(args.dist.glob("legends_stable_audio_3-0.1.0-*.whl"))
    sdists = sorted(args.dist.glob("legends_stable_audio_3-0.1.0.tar.gz"))
